...
function findHarmonicallyRelevantChords(chord) {
    const relevantChords = [...chordSets.MAJ1 /*, include other chord sets here */].filter(item => item.chord.includes(chord) || item.chord.includes(chord.replace('b', '#')) || item.chord.includes(chord.replace('#', 'b')) || item.chord.includes(chord.replace(/mi|sus|add/, '')));
...
function findChordsForKey(key) {
    ...
    if (typeof harmonics === 'string') {
        return { error: harmonics };
    }
...
app.post('/process', (req, res) => {
    ...
    const result = findChordsForKey(input);
    res.json(result);
});
