...
function findHarmonicallyRelevantChords(chord) {
  const relevantChords = Object.values(chordSets).flat().filter(item => item.name.includes(chord) || item.name.includes(chord.replace('b', '#')) || item.name.includes(chord.replace('#', 'b')) || item.name.includes(chord.replace(/mi|sus|add/, '')));
...
rl.question("Enter the root key (e.g., A, Bb, C#): ", (root) => {
  if (root) {
    ...
    rl.question("Which chord are you on? (e.g., Eb, F#add9, Cmi7): ", (currentChord) => {
      if (currentChord) {
        console.log(findChordsForKey(currentChord));
      }
      rl.close();
    });
  } else {
